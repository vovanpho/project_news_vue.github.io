<template>
  <div class="permanent">
    <form class="formInline mlauto">
      ...
      <button href="#" class="btn btnmd" type="submit">
        <fontAwesomeIcon :icon="['fas', 'search']" />
      </button>
      <div class="closeSearch" v-if="query.length > 0" @click="reset">
    ...
    <div class="viewResult" v-if="query.length > 0 && searchResultVisible">
      ...
      <div v-if="searchResults.length == 0">
        <p>
          No resutl for "<strong>{{ query }}</strong
          >"
        </p>
      </div>
    </div>
...
export default {
  ...
  computed: {
    posts() {
      return this.$store.hasModule("modEverything").state.items;
    }
  },
  ...
  methods: {
    ...
    scrollIntoView() {
      this.$refs.results.children[this.highlightIndex].scrollIntoView({
        block: "nearest"
      });
    },
    gotolink() {
